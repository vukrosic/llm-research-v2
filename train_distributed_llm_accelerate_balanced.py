# =============================================================================
# DISTRIBUTED TRAINING WITH HUGGING FACE ACCELERATE - OPTIMIZED FOR RTX 4090s
# =============================================================================
# This version specifically addresses GPU imbalance issues
# Optimized for 2x RTX 4090 (24GB VRAM each)
# =============================================================================
...
class ModelConfig:
    ...
    d_model: int = 512              # Slightly larger for better GPU utilization
    n_heads: int = 8
    num_kv_heads: Optional[int] = None
    n_layers: int = 8               # More layers for better distribution
    d_ff: int = 2048                # Larger feed-forward
    batch_size: int = 16            # Balanced batch size for 4090s
    max_steps: int = 3000           # More steps for better convergence

    # Training parameters
    gradient_accumulation_steps: int = 2  # Reduced for better balance
    learning_rate: float = 0.001

    # Data parameters
    max_seq_len: int = 1024         # Longer sequences for better GPU utilization
    num_documents: int = 3000       # More data
    max_tokens: int = 800000        # More tokens
...
class MuonOptimizer(torch.optim.Optimizer):
    """Muon optimizer - optimized for better GPU balance"""
    ...
    if os.path.exists(cache_file):
        ...
        for i, item in enumerate(dataset):
            if i >= config.num_documents:
                break
            texts.append(item["text"][:4000])  # Longer texts for better GPU utilization
...
def train_model(config: ModelConfig, train_loader: DataLoader, val_loader: DataLoader, accelerator: Accelerator):
    """Train the model with Accelerate - optimized for GPU balance"""
    
    logger.info(f"\n🚀 Training Balanced model with Accelerate")
...
def main():
    """Main training function - optimized for RTX 4090 balance"""
    
    # Initialize Accelerate with specific settings for RTX 4090s
    accelerator = Accelerator(
        mixed_precision="fp16",  # Use fp16 for better performance on 4090s
        gradient_accumulation_steps=2,  # Reduced for better balance
        log_with="tensorboard",
        project_dir="accelerate_logs",
        # Force specific device placement for better balance
        kwargs_handlers=[torch.cuda.amp.GradScaler()] if torch.cuda.is_available() else []
    )
    
    # Log configuration
    if accelerator.is_main_process:
        logger.info("=" * 80)
        logger.info("�� BALANCED DISTRIBUTED LLM TRAINING WITH ACCELERATE")
        logger.info("�� OPTIMIZED FOR RTX 4090s - ADDRESSING GPU IMBALANCE")
        logger.info("=" * 80)
        logger.info(f"🔧 Device: {accelerator.device}")
        logger.info(f"�� Mixed precision: {accelerator.mixed_precision}")
    ...
    # Create config optimized for RTX 4090s
    config = ModelConfig()
    if accelerator.is_main_process:
    ...
    # Create data loaders with balanced settings
    train_loader = DataLoader(
        train_dataset, 
        batch_size=config.batch_size,
        shuffle=True,
        num_workers=4,  # Balanced workers
        pin_memory=True,
        drop_last=True,
        persistent_workers=True  # Keep workers alive for better balance
    )
