...
def load_and_cache_data(config: ModelConfig, accelerator: Accelerator, cache_dir: str = "data_cache"):
    ...
    if os.path.exists(cache_file):
        ...
        accelerator.print(f"✅ Loaded {len(texts)} documents, {len(tokens):,} tokens from cache")
        return texts, tokenizer, tokens

    # Only main process downloads and processes
    if accelerator.is_main_process:
        accelerator.print(f"🔄 Processing new data (will cache for future use)")
        ...
        accelerator.print(f"Loaded {len(texts)} documents")

        # Tokenize
        accelerator.print("Tokenizing texts...")
        all_tokens = []
        for text in tqdm(texts, desc="Tokenizing", disable=not accelerator.is_local_main_process):
            tokens = tokenizer.encode(text, add_special_tokens=False)
            all_tokens.extend(tokens)

        tokens = all_tokens[:config.max_tokens]
        accelerator.print(f"Using {len(tokens):,} tokens")
        config.vocab_size = tokenizer.vocab_size
